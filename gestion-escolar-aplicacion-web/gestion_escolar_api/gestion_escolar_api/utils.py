...
class Utils:
    ...
    @staticmethod
    def missingRequiredFields(data, required_fields):
        missing = []
    ...
    @staticmethod
    def isAlphanumericNoSpaces(value):
        if value is None:
            return False
        return bool(re.fullmatch(r"[A-Za-z0-9]+", str(value)))

    @staticmethod
    def isDigitsNoSpaces(value):
        if value is None:
            return False
        return bool(re.fullmatch(r"[0-9]+", str(value)))

    @staticmethod
    def isLettersOnly(value):
        if value is None:
            return False
        return bool(re.fullmatch(r"[A-Za-zÁÉÍÓÚÜÑáéíóúüñ]+", str(value)))
